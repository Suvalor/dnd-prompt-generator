...
**本次需求核心**：在保留现有浏览器端确定性生成器作为 fallback 的前提下，引入后端驱动的 LLM（MiMo）文本生成能力，并构建完整的安全骨架（配额控制、滥用防护、密钥隔离）和多模态预留接口（图片/视频分析）。
...
| **使用场景** | 访客填写角色/场景表单 → 后端调用 MiMo API 生成提示词 → 复制粘贴到图像模型 |
| **商业约束** | 10次/小时配额是硬性成本限制，用于控制 MiMo API 费用 |
...
| P0.3 | 安全骨架 — API 认证 | MiMo API 密钥通过环境变量注入，不硬编码 | 密钥不暴露在代码/日志中 |
| P0.4 | MiMo 文本生成 | 后端调用 MiMo API 生成 D&D 提示词 | 返回格式与现有前端兼容，包含 main/short/negative/style/usage |
...
| # | 功能 | 描述 | 验收标准 |
|---|------|------|----------|
| P2.1 | 图片分析接口 | 上传图片 → MiMo 分析 → 返回提示词建议 | 接口设计完成，可空实现 |
| P2.2 | 视频分析接口 | 上传视频 → MiMo 分析 → 返回提示词建议 | 接口设计完成，可空实现 |
...
### AC-3: Fallback 降级
```gherkin
Given MiMo API 密钥缺失或 API 返回 5xx
When 用户发起生成请求
Then 后端返回 HTTP 200
...
### Sprint 2: MiMo 文本生成（Phase 2）
- **目标**：MiMo API 集成 + Fallback 机制 + 前端适配
- **交付物**：
  - MiMo API 客户端封装
...
| 风险 | 影响 | 缓解措施 |
|------|------|----------|
| **R1: MiMo API 稳定性** | 高 | Fallback 机制确保服务可用；监控 API 健康状态 |
...
| `/api/generate-prompt` | POST | 现有（DeepSeek） | 需迁移到 MiMo，添加 fallback |
